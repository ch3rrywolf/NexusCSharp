/*
   ...
   Tutorial: Use C# to match data against patterns.
    1. Match a value.
    2. Enum matches.
    3. Exhaustive matches with switch.
    4. Type patterns.
*/
...
if (transactionType?.ToUpper() is "DEPOSIT")currentBalance += amount;
...
// _=> 0.0,
};  // warning CS8509: The switch expression does not handle all possible values of its input type (it is not exhaustive). For example, the pattern '(TransactionType.Invalid, _)' is not covered.
Console.WriteLine($"{transaction.type} => Parsed Amount:{transaction.amount}, New Balance: {currentBalance}");
}   //The compiler issues a warning. The test data is valid, so the program works.
    //  However, any invalid data would cause a failure at runtime.
public enum TransactionType
{
Deposit,
Withdrawal,
